...
class OtherCommands(commands.Cog):
    ...
    @commands.command(name="testme")
    @commands.has_permissions(manage_roles=True)
    @commands.bot_has_permissions(manage_roles=True)
    async def testme_command(self, ctx, member: nextcord.Member):
        """
        testme command will remove the Visitors role and replace it with the
        Members role for the targetted member.  It will also send a testme message (messages/testme.txt)
        to the member_join_channel (retrieved from DB)
        ...
        if channel_id is None:
            await owner.send(
                "(testme_command) There was an error retrieving the channel ID (member_join_channel) from DB"
            )
        else:
            member_ch = self.bot.get_channel(channel_id)
            guild = ctx.guild
            ...
            if botchannel_id is None or reactionrole_ch_id is None:
                await owner.send(
                    "(testme_command) There was an error retrieving the channel ID (botchannel_id) or (reactionrole_ch_id)  from DB"
                )
                ...
                """
              Check member for old role.  If exists, remove old role and add new role,
              then send testme message to member join channel
              """
                #if old_role in member.roles:
                await member.remove_roles(old_role)
                await member.add_roles(new_role)
    ...
    @testme_command.error
    async def testme_command_error(self, ctx, error):
        """
        Error check, if command user or bot is missing permission, error message
        sent to command channel.
        Command and error message are auto-deleted after 30 seconds to keep
